...
M0=dens.*vp.^2.*(cos(0.5*atan(1./Qp))).^2;
Clam=M0./wo.^(erfa);


sitap=M0.*cos(pi.*rp).*(1-rps);
yindap=Clam.*vp.^(erfa).*cos(pi.*rp).*rps;
taop=M0.*vp.^(-1).*sin(pi.*rp).*(1-rps);
beitap=Clam.*vp.^(erfa-1).*sin(pi.*rp).*rps;


pw1=k2w.^(0.5*erfa);
pw3=k2ww.^(0.5*erfa-0.5);
pw2=k2ww.^(-0.5);
...
%re=zeros(tmax,nx);
recx=fopen('trace_chen.dat','wb');

fx=fopen('snap_chen.dat','wb');
...
%      end
     ...
     md0p=sitap.*(real(ifft2(fft_mid3)));
     md1p=yindap.*(real(ifft2(pw1.*fft_mid3)));
     md2p=taop.*(real(ifft2(pw2.*fft_mid3))-real(ifft2(pw2.*midxxzz_f)))/dt;
     md3p=beitap.*(real(ifft2(pw3.*fft_mid3))-real(ifft2(pw3.*midxxzz_f)))/dt;
     % md3p=beitap.*(real(ifft2(pw3.*midf)));

     midxxzz_f=fft_mid3;

     p=p+dt*(md0p+md1p+md2p+md3p);
