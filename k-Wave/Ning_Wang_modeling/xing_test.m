...
%re=zeros(tmax,nx);
recx=fopen('trace_xing.dat','wb');

fx=fopen('snap_xing.dat','wb');
...
%      end
     ...
     md1p=C1.*(real(ifft2(pw1.*fft_mid3)));
     md2p=C2.*(real(ifft2(fft_mid3)));
     md3p=C3.*(real(ifft2(pw2.*fft_mid3)));
     
     md1tp=C4.*(real(ifft2(pw1.*fft_mid3))-real(ifft2(pw1.*midxxzz_f)))/dt;
     md2tp=C5.*(real(ifft2(fft_mid3))-real(ifft2(midxxzz_f)))/dt;
     

     midxxzz_f=fft_mid3;

     p=p+dt*(md1p+md2p+md3p+md1tp+md2tp);
