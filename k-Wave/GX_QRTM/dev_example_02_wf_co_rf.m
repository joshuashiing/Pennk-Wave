...
% for i = 15 : 15
    ...
    tmp1 = load(datafile);
    datafile = [ex_name '/CSG_ac_sm_' num2str(i, '%.3i') '.mat'];
    tmp2 = load(datafile);
    ...
    wavefile = [ex_name '/WF_co_rf_' num2str(i, '%.3i') '.mat'];
    save(wavefile, 'p_save_fm', 'p_save_bp');
end
...
% mig = zeros(Nx, Ny);
% for i = 1 : (n - 1)
%     mig = p_save_fm(:, :, i) .* p_save_bp(:, :, (n-i)) + mig;
% end
% imagesc(mig)
% % imagesc(del2(mig(20:end, :))); colorbar;
