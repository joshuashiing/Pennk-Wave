...
wf_file = 'Data_example_01/WF_cd_ac_010.mat';
load(wf_file);
n = size(p_save_fm, 2);
...
p_k = fft2(p);
filter_u = kgrid_tmp.kx_vec .* (kgrid_tmp.kx_vec >= 0);
filter_d = kgrid_tmp.kx_vec .* (kgrid_tmp.kx_vec <= 0);
p_k_u = ifftshift(filter_u) .* p_k;
p_k_d = ifftshift(filter_d) .* p_k;
p_u = ifft2(p_k_u);
p_d = ifft2(p_k_d);

subplot(311); imagesc(p, [-clim, clim]); colorbar;
subplot(312); imagesc(abs(p_u), [-clim, clim]); colorbar;
subplot(313); imagesc(abs(p_d), [-clim, clim]); colorbar;
% subplot(234); imagesc(p_k); colorbar;
% subplot(235); imagesc(p_k); colorbar;
% subplot(236); imagesc(p_k); colorbar;