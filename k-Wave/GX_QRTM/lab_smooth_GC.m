...
[d0, t_axis] = heter_simu(Nx, Ny, dx, dy, f0_model, f0, c0, Q, rho, ...
...
for j = 1 : length(sigma_list)
    sigma = sigma_list(j);
    c0_sm = imgaussfilt(c0, sigma);
    Q_sm = imgaussfilt(Q, sigma);
    rho_sm = imgaussfilt(rho, sigma);
    [d(:, :, j), t_axis] = heter_simu(Nx, Ny, dx, dy, f0_model, f0, c0_sm, Q_sm, rho_sm, ...
                    stf, x_src(i), y_src(i), x_rec, y_rec, ...
                    dt, t_max, mod_mech, -1, '', args);
end
...
figure(2);
for j = 1 : length(sigma_list)
    plot(t_axis, d0(r, :) - d(r, :, j), [clist{j} '--'], 'linewidth', 2); hold on;
end
