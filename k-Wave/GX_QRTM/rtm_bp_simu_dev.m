function [d, p_save, t_axis] = rtm_bp_simu_dev(Nx, Ny, dx, dy, model_f0_m, model_f0, ...
    model_c0_m, model_Q, model_density, x_rec, y_rec, ...
    dt, t_max, mod_mech, f_cutoff, taper_ratio, t_snap, mat_name, p_data, args)
...
% =========================================================================
% RTM Q-compensated Backpropagation
% =========================================================================

sensor.time_reversal_boundary_data = p_data;
medium.alpha_sign = [1, -1];
medium.alpha_filter = getAlphaFilter_GXtmp(kgrid, medium, f_cutoff,taper_ratio);
medium.save_ndt = 1;
medium.save_pressure = true;


if exist('args', 'var')
    [d, p_save] = kspaceFirstOrder2D(kgrid, medium, 1, sensor, args{:});
else
    [d, p_save] = kspaceFirstOrder2D(kgrid, medium, 1, sensor);
end
