...
% check medium fields
checkFieldNames(medium, {'sound_speed', 'sound_speed_ref', 'density', 'alpha_coeff', ...
    'alpha_power', 'alpha_mode','alpha_model', 'alpha_filter', 'alpha_sign', 'BonA','qualityfactor','Lrelax'});
...
%     end
    ...
    % check the absorption model input is valid
    if isfield(medium, 'alpha_model')
        if ~ischar(medium.alpha_model) || (~strcmp(medium.alpha_model, 'CQ') ...
                && ~strcmp(medium.alpha_model, 'KVQ') && ~strcmp(medium.alpha_model, 'ZQ'))
            error('medium.alpha_model must be set to ''CQ'' or ''KVQ'' or "ZQ" ');
        end
    end  
    ...
    if isfield(medium, 'alpha_sign') && (~isnumeric(medium.alpha_sign) || numel(medium.alpha_sign) > 3)
        error('medium.alpha_sign must be given as a 3 element numerical array')
    end    
    
elseif isfield(medium, 'qualityfactor')
    equation_of_state = 'absorbing-SLS'; 
...
if ~isempty(sensor)
    ...
    if ~isa(sensor, 'kWaveTransducer')
        if kgrid.dim == 2
            ...
            checkFieldNames(sensor, {'mask','mask0', 'directivity_pattern', 'directivity_angle', 'directivity_size',...
...
elseif ~isa(source, 'kWaveTransducer')
    ...
    switch kgrid.dim
        case 1
            checkFieldNames(source, {'f0','p0', 'p', 'p_mask', 'p_mode', 'ux', 'u_mask', 'u_mode'});
        case 2
            checkFieldNames(source, {'f0','p0', 'p', 'p_mask', 'p_mode', 'ux', 'uy', 'u_mask', 'u_mode'});
        case 3
            checkFieldNames(source, {'f0','p0', 'p', 'p_mask', 'p_mode', 'ux', 'uy', 'uz', 'u_mask', 'u_mode'});
    end
    ...
    if isfield(source, 'p')
        ...
        if isfield(source, 'p_mode')
            if ~ischar(source.p_mode) || (~strcmp(source.p_mode, 'additive') && ~strcmp(source.p_mode, 'dirichlet') && ~strcmp(source.p_mode, 'analytic'))
...
return_field = RETURN_FIELD_DEF;
smooth_ani = SMOOTH_RHO0_DEF;
...
elseif ~isempty(varargin)
    for input_index = 1:2:length(varargin)
        switch varargin{input_index}           
            ...
            case 'ReturnField'
                return_field = varargin{input_index + 1};
                if ~islogical(return_field)
                    error('Optional input ''ReturnField'' must be Boolean');
               end       
            case 'StreamToDisk'
                if kgrid.dim == 3
