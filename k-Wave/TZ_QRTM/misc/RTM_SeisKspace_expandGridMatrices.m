...
if isfield(medium, 'alpha_model') && numel(medium.alpha_power) > 1
    medium.alpha_power = expandMatrix(medium.alpha_power, expand_size);
end

if isfield(medium, 'qualityfactor') && numel(medium.qualityfactor) > 1
    medium.qualityfactor = expandMatrix(medium.qualityfactor, expand_size);
end
...
if isfield(medium, 'Delta');
    medium.Delta = expandMatrix(medium.Delta, expand_size, 0);
end 

if isfield(medium, 'Epsilon');
    medium.Epsilon = expandMatrix(medium.Epsilon, expand_size, 0);
end 
