...
if p_source
    ...
    elseif strcmp(source.p_mode, 'analytic')
        
        % compute the scale parameter seperately for each source position
        % based on the sound speed at that position
        if strcmp(equation_of_state, 'absorbing-SLS')
            if numel(c) == 1
                % compute the scale parameter based on the homogeneous sound speed
                source.p = source.p.*c.^2.*dt/kgrid.dx^N./ (N*c.^2);
            else
                for p_index = 1:length(source.p(:,1))
                    source.p(p_index, :) = source.p(p_index, :).*MR(ps_index(p_index))...
                        ./rho0(ps_index(p_index)).*dt/kgrid.dx^N./ (N*c(ps_index(p_index)).^2);
                end
            end
        else
            %             for p_index = 1:length(source.p(:,1))
            %                 source.p(p_index, :) = source.p(p_index, :).*c.^2.*dt/kgrid.dx^N./ (N*c.^2);
            %             end
            if numel(c) == 1
                % compute the scale parameter based on the homogeneous sound speed
                source.p = source.p.*c.^2.*dt/kgrid.dx^N./ (N*c.^2); 
                ...
                % based on the sound speed at that position
                for p_index = 1:length(source.p(:,1))
                    source.p(p_index, :) = source.p(p_index, :).*c(ps_index(p_index)).^2.*dt/kgrid.dx^N./ (N*c(ps_index(p_index)).^2);
                end
        ...
        if numel(c) == 1
            ...
            % based on the sound speed at that position
            for p_index = 1:length(source.p(:,1))
                source.p(p_index, :) = source.p(p_index, :) .*c(ps_index(p_index)).^2.* (2.*dt./(N*c(ps_index(p_index)).*kgrid.dx));
            end
        end
    end
end
